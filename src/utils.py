    ...
    def process_set(self, dataset):
        ...
        with tqdm(file_list) as t:
            for file_name in t:
                ...
                # print(type(data))
                # print("data.pos.shape: {}".format(data.pos.shape))
                # print("data.norm.shape: {}".format(data.norm.shape))
                # print(data.shape)
                data.y = torch.tensor([cate_id[category]])
                # print("[cate_id[category]]: {}".format([cate_id[category]]))
                # print(type(data.y))
                # print(data.y)
                '''
                <class 'torch_geometric.data.data.Data'>
                data.pos.shape: torch.Size([10000, 3])
                data.norm.shape: torch.Size([10000, 3])
                [cate_id[category]]: [0]
                <class 'torch.Tensor'>
                tensor([0])
                '''
